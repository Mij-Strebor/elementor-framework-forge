...
- **Elementor kit CSS auto-regeneration** — When `eff_sync_from_elementor` is called and the kit CSS file does not exist on disk, EFF now attempts to regenerate it via Elementor's `CSS\Post` API before returning an error. This prevents the 0-variable result that occurred on fresh installs or after Elementor clears its CSS cache, eliminating the need to load a page in the browser first.
